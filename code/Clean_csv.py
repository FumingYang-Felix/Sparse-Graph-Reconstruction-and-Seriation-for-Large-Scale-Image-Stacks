"""
Clean pairwise-alignment CSV (filter + score) after GCD algorithm — quick preprocessing utility.

What it does
------------
1) Drops rows where `ssim == -1` (invalid matches).
2) Keeps only rows with near-unity scale: `0.9 <= scale <= 1.1`.
3) If a `num_inliers` column exists, adds a new `score` column:
      score = ssim * num_inliers
   (If `num_inliers` is missing, score computation is skipped.)

Required / optional columns
---------------------------
Required:  ssim, scale
Optional:  num_inliers, fixed, moving (and any other columns you may have;
           they are preserved and passed through unchanged).

Input / Output
--------------
• Input:  a CSV file (comma-separated) with the columns above.
• Output: a cleaned CSV written to:
    - the path given via `-o/--output`, or
    - by default:  <project_root>/results/sequencing/cleaned_csv/<input_basename>_cleaned.csv

Console output includes pre/post row counts, filter summaries, and basic stats.

Usage
-----
Basic:
    python3 clean_csv.py path/to/pairs.csv

Specify output path:
    python3 clean_csv.py path/to/pairs.csv -o path/to/cleaned.csv

Notes
-----
• This script never edits the input file; it writes a new CSV.
• To change scale bounds, modify the two comparisons near:
      df = df[(df['scale'] >= 0.9) & (df['scale'] <= 1.1)]
• If required columns are missing, the script prints a warning and exits early.
• Depends on pandas. Install with:
      pip install pandas
"""
